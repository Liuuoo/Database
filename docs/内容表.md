...
这样的效率足以与红黑树、B+树等复杂数据结构媲美。所以跳表也是被了LevelDB,redies等数据库使用。
...
### CURD
我们暂定，使用GET(key)进行主键搜索、PUT({key,value})进行新增与修改数据、DELETE进行删除数据。这边需要详细介绍他们的作用机制。
1. GET(KEY)- 这边仅仅是先实现到KEY的查找，不支持别的索引，后续再更新。
2. PUT({key,velue})- 先使用搜索，找到位置，然后将新的Key-value放置到原位置前面的位置，这样就实现了既能新增又能修改。由于主键唯一，后续依据该原理进行清理。
3. DELETE(key)- 这边预计使用墓碑机制，为每个KEY增加一个额外字段，将其设置为0或NULL?不过关于SSTable的清理仍然交给后续设计，这边不作规划。

### 快照
加入额外序列号字段，当用户保存快照时，记录一个值（如30），那么当用户恢复时，只需要判定`<=30`的元素即可。当我们清理时，`<=30`的墓碑等都不应该被清理。

### 模块划分
#### 对外接口层
该层主要处理如下内容：
- 处理识别 GET、PUT、DELETE等请求，与返回的请求
- 输出系统的状态，日志

当用户发出请求时，该层主要检查KV语句完整性（使用CLI操作），然后调用信息处理模块进行处理信息，并将元信息传递给下一层。

用户可以使用特定语句调出数据库状态（内存、磁盘使用情况）与日志。

#### 内存表模块
该层主要负责MemTable也就是我们维护的跳表
- 负责开辟Arena内存池，负责内存池的内存分配
- 跳表的维护，包括查询与增加元素。
- 负责管理MemTable与冻结 MemTable，但不进行Immutable Table的落盘，不过负责Immutable Table的调度。

#### 磁盘模块
该层主要负责磁盘中的各个功能
- WAL的写入，数据的恢复
- 负责磁盘文件命名、打开、关闭、顺序读写这些 I/O 行为。
- 负责SSTable的落盘，合并，清理。

#### 记录结构与元信息
- 负责定义系统里“记录到底长什么样”。
- 这里要统一 key、value、操作类型、tombstone、序列号/时间戳、比较规则、编码格式。
- 这是 MemTable、WAL、SSTable 共享的数据契约。
- 不负责保存数据，不负责调度流程，可以理解为数据处理工具的作用。

### 具体实现
这边强制Command语句为PUT\GET\DELETE+' '+{key,value}。
#### 对外接口层
这是整个项目的开始，主要负责处理cli中收到的信息，将信息压缩为commond，然后传递给memtable。该层主要位于.\include\interface中。以头文件的形式存在。各个头文件项目调用。该层大致的任务以及执行流程如下：
```
      用户输出关键字(PUT\GET\DELETE)+{key,value}的信息
                     |
                     |
   将字符串传递给parser，进行切割，增加字段，拼接成command
                     |
                     |
               处理好的command 
                     |
                     |
    parser提供get_command方法传出处理好的command
```
...
{
    opType type;
    uint8_t key_size; // key的大小
    uint8_t value_size; // value的大小
    std::string_view key;
    std::string_view value; // value
    uint32_t timestamp; // 时间戳
    bool tombstone_flag; // 墓碑标志
}; 
```
我们先做好PUT的command,等到核心内存结构实现了，有了查找函数后，再通过SEARCH出来其余的值，然后拼成一个command。

最终效果为：
```
C:\Users\LIU\Desktop\Project\Database>.\build\bin\test1.exe                                     
PUT {123,1231}
Command Type: 2
Key Size: 3
Value Size: 4
...
Value: 1231
Timestamp: 69607598
Tombstone Flag: false
```
至此，对外接口层简易版几乎是完成，当然这边实际上有很多的问题，尤其是这个异常处理问题，还有很多需要补充的。

#### statu
这边还是准备补一个状态码，将之封装为一个类，使用枚举类型来表示，美观便于开发且开销小。这边将我现在能考虑到的异常情况给列举出来。这边参照静态工厂模式完成这个status类。先补一点静态工厂模式的知识。
##### 静态工厂模式
这边先贴出代码，然后照着代码分析即可：
```cpp
#pragma once
...
private:
   ...
   std::string msg_;
```
- 将一个完整的构造函数写入，显然外部是无法在创建实体的时候传入两个参数了。
- 外部想要创建一个status需要使用static的函数方法通过方法return一个构造方式完成。
   - 这样能直接返回选择的一种写死的状态，而不是自定义的状态。
- 将字段写在后面是一种风格方式，为了先将逻辑展现，然后展现字段。
...
```
当后续想使用ToString函数时，会通过静态函数返回的构造方法得到的code_去判定，总之这个架构设计的很巧妙。

我们再来看开头，定义一个枚举类型，然后可以枚举四个变量值。
```cpp
enum Code {
   kOk = 0,
   kNotFound = 1,
...
```
#### WAL
我们刚刚写了status类，现在我们做wal，并将status应用进来。
我们将这些command落盘，做成WAL，只有正确的响应后才进行下一步，否则提醒用户操作失败，让他重新输入。在这一步我们不进行wal的更新，准备到SSTable落盘后，再着手完整的WAL。

#### skiplist
我认为接下来我要开始着手于skiplist的实现，然后将其封装作为memstable。
我们的skiplist的每一个节点都应该是一个command，但我们应该多出来一些东西，我的想法是将节点封装为一个类来进行处理，而不再使用结构体的方式，也方便后续方法的加入。
